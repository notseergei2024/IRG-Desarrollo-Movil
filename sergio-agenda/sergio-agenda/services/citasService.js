...
export async function listarCitas() {
  return DB.leer(DB.KEY_CITAS);
}

/** Devuelve las citas que coincidan con el filtro dado. */
export async function listarCitasFiltradas({
  empleadosActivos = null,   // array de ids o null para no filtrar
  incluirAnuladas = false,
  fecha = null               // 'YYYY-MM-DD' o null para todas
} = {}) {
...
export async function crearCita(data) {
  ...
  todas.push(cita);
  await DB.escribir(DB.KEY_CITAS, todas);
  dbEvents.emit();
  return cita;
}
 ...
 */
export async function anularCita(id) {
