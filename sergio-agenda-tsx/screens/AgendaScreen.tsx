...
type MarkedDates = Record<string, MarcaCalendario>;
...
export default function AgendaScreen(): JSX.Element {
  ...
  const [verAnuladas, setVerAnuladas] = useState<boolean>(false);
  const [verOtrasOficinas, setVerOtrasOficinas] = useState<boolean>(false);

  // ---- Estado de modales ----
  const [showNuevaCita, setShowNuevaCita] = useState<boolean>(false);
  const [editandoCita, setEditandoCita] = useState<Cita | null>(null); // null = crear
  ...
  const citasVisibles = useMemo<Cita[]>(() => {
  ...
  const citasDelDia = useMemo<Cita[]>(() => {
  ...
  // ---- Acciones ----
  const toggleEmpleado = (id: string): void => {
  ...
  const toggleTodos = (activar: boolean): void => {
    setEmpleadosActivos(activar ? EMPLEADOS.map((e) => e.id) : []);
  };

  const abrirNuevaCita = (): void => {
    setEditandoCita(null);
    setShowNuevaCita(true);
  };

  const abrirEditarCita = (cita: Cita): void => {
    setEditandoCita(cita);
    setShowNuevaCita(true);
  };

  // Mapa rápido id → cliente.
  const clientesPorId = useMemo<Record<string, Cliente>>(() => {
...
function Opcion({ label, activo, onToggle }: OpcionProps): JSX.Element {
  return (
    <TouchableOpacity
